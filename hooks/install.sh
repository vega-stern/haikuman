#!/bin/bash
# haikuman — one-command hook installer for Claude Code
# Installs: SessionStart hook (auto-load rules) + UserPromptSubmit hook (mode tracking)
# Usage: bash hooks/install.sh
#   or:  bash <(curl -s https://raw.githubusercontent.com/vega-stern/haikuman/main/hooks/install.sh)
...
if ! command -v node >/dev/null 2>&1; then
  echo "ERROR: 'node' is required to install the haikuman hooks (used to merge"
...
REPO_URL="https://raw.githubusercontent.com/vega-stern/haikuman/main/hooks"
...
if [ "$FORCE" -eq 0 ]; then
  ...
  if [ "$ALL_FILES_PRESENT" -eq 1 ] && [ -f "$SETTINGS" ]; then
    if HAIKUMAN_SETTINGS="$SETTINGS" node -e "
      const fs = require('fs');
      const settings = JSON.parse(fs.readFileSync(process.env.HAIKUMAN_SETTINGS, 'utf8'));
      const hasHook = (event) =>
      ...
      process.exit(
        hasHook('SessionStart') &&
        hasHook('UserPromptSubmit') &&
        !!settings.statusLine
          ? 0
          : 1
      );
  ...
  if [ "$ALL_FILES_PRESENT" -eq 1 ] && [ "$HOOKS_WIRED" -eq 1 ] && [ "$HAS_STATUSLINE" -eq 1 ]; then
    ALREADY_INSTALLED=1
    echo "Haikuman hooks already installed in $HOOKS_DIR"
...
if [ "$FORCE" -eq 1 ] && [ -f "$HOOKS_DIR/haikuman-activate.js" ]; then
  echo "Reinstalling haikuman hooks (--force)..."
else
  echo "Installing haikuman hooks..."
fi
...
chmod +x "$HOOKS_DIR/haikuman-statusline.sh"
...
HAIKUMAN_SETTINGS="$SETTINGS" HAIKUMAN_HOOKS_DIR="$HOOKS_DIR" node -e "
  ...
  // SessionStart — auto-load haikuman rules
  ...
  // UserPromptSubmit — track mode changes when user types /haikuman commands
  ...
  // Statusline — wire haikuman badge (report if skipped)
...
echo "  - SessionStart hook: auto-loads haikuman rules every session"
echo "  - Mode tracker hook: updates statusline badge when you switch modes"
echo "    (/haikuman lite, /haikuman ultra, /haikuman-commit, etc.)"
echo "  - Statusline badge: shows [HAIKUMAN] or [HAIKUMAN:ULTRA] etc."
