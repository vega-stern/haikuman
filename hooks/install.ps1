# haikuman — one-command hook installer for Claude Code (Windows PowerShell)
...
#        irm https://raw.githubusercontent.com/vega-stern/haikuman/main/hooks/install.ps1 | iex
...
if (-not (Get-Command node -ErrorAction SilentlyContinue)) {
    Write-Host "ERROR: 'node' is required to install the haikuman hooks (used to merge" -ForegroundColor Red
...
$RepoUrl = "https://raw.githubusercontent.com/vega-stern/haikuman/main/hooks"

$HookFiles = @(
    "haikuman-config.js",
    "haikuman-activate.js",
    "haikuman-mode-tracker.js",
    "haikuman-statusline.sh",
    "haikuman-statusline.ps1"
)
...
if (-not $Force) {
    ...
    if ($AllFilesPresent -and (Test-Path $Settings)) {
        try {
                ...
                foreach ($entry in $entries) {
                    if ($entry.hooks) {
                        foreach ($hookDef in $entry.hooks) {
                            if ($hookDef.command -and $hookDef.command.Contains("haikuman")) {
                                return $true
                            }
                        }
                    }
            ...
            $HooksWired = (& $hasHaikumanHook "SessionStart") -and (& $hasHaikumanHook "UserPromptSubmit")
    ...
    if ($AllFilesPresent -and $HooksWired -and $HasStatusLine) {
        Write-Host "Haikuman hooks already installed in $HooksDir"
...
if ($Force -and (Test-Path (Join-Path $HooksDir "haikuman-activate.js"))) {
    Write-Host "Reinstalling haikuman hooks (-Force)..."
} else {
    Write-Host "Installing haikuman hooks..."
}
...
$env:HAIKUMAN_SETTINGS = $Settings -replace '\\', '/'
$env:HAIKUMAN_HOOKS_DIR = $HooksDir -replace '\\', '/'

$nodeScript = @'
const fs = require('fs');
const settingsPath = process.env.HAIKUMAN_SETTINGS;
const hooksDir = process.env.HAIKUMAN_HOOKS_DIR;
const managedStatusLinePath = hooksDir + '/haikuman-statusline.ps1';
...
const hasStart = settings.hooks.SessionStart.some(e =>
  e.hooks && e.hooks.some(h => h.command && h.command.includes('haikuman'))
);
if (!hasStart) {
  settings.hooks.SessionStart.push({
    hooks: [{
      type: 'command',
      command: 'node "' + hooksDir + '/haikuman-activate.js"',
      timeout: 5,
      statusMessage: 'Loading haiku mode...'
    }]
  });
}
...
if (!hasPrompt) {
  settings.hooks.UserPromptSubmit.push({
    hooks: [{
      type: 'command',
      command: 'node "' + hooksDir + '/haikuman-mode-tracker.js"',
      timeout: 5,
      statusMessage: 'Tracking haiku mode...'
    }]
  });
}
...
if (!settings.statusLine) {
  ...
  if (cmd.includes(managedStatusLinePath)) {
    ...
    console.log('  NOTE: Existing statusline detected - haikuman badge NOT added.');
...
Write-Host "  - SessionStart hook: auto-loads haikuman rules every session"
Write-Host "  - Mode tracker hook: updates statusline badge when you switch modes"
Write-Host "    (/haikuman lite, /haikuman ultra, /haikuman-commit, etc.)"
Write-Host "  - Statusline badge: shows [HAIKUMAN] or [HAIKUMAN:ULTRA] etc."
