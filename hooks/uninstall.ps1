# haikuman — uninstaller for the SessionStart + UserPromptSubmit hooks (Windows PowerShell)
...
#   or:  irm https://raw.githubusercontent.com/vega-stern/haikuman/main/hooks/uninstall.ps1 | iex
param()
...
$FlagFile = Join-Path $ClaudeDir ".haikuman-active"
...
# Detect if haikuman is installed as a plugin
$PluginInstalled = $false
$PluginsDir = Join-Path $ClaudeDir "plugins"
if (Test-Path $PluginsDir) {
    $found = Get-ChildItem -Path $PluginsDir -Recurse -Filter "plugin.json" -ErrorAction SilentlyContinue |
        Where-Object { $_.FullName -match "haikuman" }
    if ($found) { $PluginInstalled = $true }
}

if ($PluginInstalled) {
    Write-Host "Haikuman appears to be installed as a Claude Code plugin." -ForegroundColor Yellow
    Write-Host "To uninstall the plugin, run:"
    Write-Host ""
    Write-Host "  claude plugin disable haikuman" -ForegroundColor Cyan
    Write-Host ""
...
Write-Host "Uninstalling haikuman hooks..."
...
if (Test-Path $Settings) {
    if (-not (Get-Command node -ErrorAction SilentlyContinue)) {
        ...
        Write-Host "         Remove the haikuman SessionStart and UserPromptSubmit"
...
Write-Host "  npx skills remove haikuman      # Cursor, Windsurf, Cline, Copilot, etc."
Write-Host "  claude plugin disable haikuman   # Claude Code plugin"
Write-Host "  gemini extensions uninstall haikuman  # Gemini CLI"
